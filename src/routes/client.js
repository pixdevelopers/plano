...
router.get('/public/:id', async (req, res, next) => {
  const client = await Client.findById(req.params.id)
    .select('name email avatar description profile.mobile profile.isParkingAvailble profile.address profile.location profile.webSite profile.qrCode profile.description');
  if (!client) return res
    .status(404)
    .send('The client with the given ID was not found.'); 
  const services =await ServiceType.find({ client: req.params.id})
